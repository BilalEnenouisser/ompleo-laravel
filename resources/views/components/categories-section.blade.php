...
<section class="platform-section relative bg-[#212221] overflow-hidden categories-section">
    ...
    <div class="platform-container relative z-10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12 justify-center w-full max-w-[50rem] mx-auto">
            ...
            <a href="{{ route('jobs.index', ['search' => $category['search']]) }}" class="block group h-full">
                <div class="p-[1px] cursor-pointer transition-colors duration-300 h-full" style="background: linear-gradient(135deg, #165c5b, #00fadc, #165c5b); border-radius: 12px;">
                    <div class="rounded-xl flex flex-col items-center justify-center gap-2.5 h-full min-h-[120px] py-4 px-5 sm:px-6 bg-[#2b2b2b] group-hover:bg-[#383838] transition-colors duration-300 shadow-[0_20px_22px_rgba(0,0,0,0.4)]" style="border-radius: 12px;">
                        <img src="{{ asset($category['icon']) }}" alt="" class="w-9 h-9 shrink-0 object-contain pointer-events-none" aria-hidden="true">
