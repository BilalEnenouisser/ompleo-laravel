...
<body class="font-sans antialiased">
    ...
    <div class="min-h-screen bg-dark-900 flex">
            ...
            <div class="flex flex-col h-full">
                ...
                <div class="flex items-center justify-between p-6 border-b border-[#333333]">
                    <a href="{{ route('home') }}" class="flex items-center space-x-3">
                        <div class="w-10 h-10 rounded-xl bg-[#00b6b4] flex items-center justify-center p-2">
                            <img 
                                src="{{ asset('fgehr.png') }}" 
                                alt="OMPLEO" 
                        ...
                        <span class="text-xl font-bold text-[#00b6b4]">OMPLEO</span>
                    </a>
                    <button
                ...
                <div class="p-6 border-b border-[#333333]">
                    <div class="flex items-center space-x-3">
                        <div class="w-12 h-12 bg-[#00b6b4] rounded-full flex items-center justify-center text-white font-bold">
                            {{ substr(Auth::user()->name, 0, 1) }}{{ substr(Auth::user()->name, -1) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-[#f5f5f5] truncate">
                                {{ Auth::user()->name }}
                            </p>
                            <p class="text-xs text-[#cccccc] capitalize">
                                {{ Auth::user()->user_type }}
                            </p>
                        </div>
                    </div>
                ...
                <nav class="flex-1 p-4 space-y-2">
                    @if(Auth::user()->user_type === 'admin')
                        <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-[#333333] text-[#00b6b4] border-r-2 border-[#00b6b4]' : 'text-[#cccccc] hover:bg-[#333333] hover:text-[#00b6b4]' }}">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="flex-1 flex flex-col lg:ml-0">
            <!-- Top Bar -->
            <header class="bg-[#2b2b2b] shadow-sm border-b border-[#333333] px-6 py-4">
            ...
            <!-- Page Content -->
            <main class="flex-1 p-6 overflow-auto bg-[#1f1f1f]">
                @yield('content')
            </main>
        </div>
