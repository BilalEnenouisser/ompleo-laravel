...
<body class="font-sans antialiased min-h-screen bg-[#1f1f1f] dark">
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button onclick="deleteAllNotifications()" class="flex items-center gap-2 px-4 py-2 bg-red-900/20 text-red-400 rounded-lg hover:bg-red-900/30 transition-colors">
                ...
                <div class="bg-[#2b2b2b] rounded-2xl p-6 shadow-lg border border-[#333333] mb-6">
                    <form id="filterForm" method="GET" action="{{ route('recruiter.notifications') }}">
                        <div class="flex flex-col md:flex-row gap-4">
                            ...
                            <div class="relative">
                                ...
                                <select id="filterSelect" name="filter" class="pl-10 pr-8 py-3 border border-[#333333] rounded-lg focus:ring-2 focus:ring-[#00b6b4] focus:border-[#00b6b4] outline-none appearance-none bg-[#2b2b2b] text-[#f5f5f5] min-w-[150px]">
                ...
                @if($notifications->hasMorePages())
                    <div class="mt-6 text-center">
                        <button id="loadMoreBtn" class="px-6 py-3 bg-[#00b6b4] hover:bg-[#009999] text-white rounded-lg transition-colors font-medium">
                            Charger plus
                        </button>
                    </div>
                @endif
    ...
    <script>
        ...
        document.getElementById('loadMoreBtn')?.addEventListener('click', function() {
            ...
            fetch(`{{ route('recruiter.notifications') }}?page=${currentPage + 1}&search=${encodeURIComponent(search)}&filter=${encodeURIComponent(filter)}`, {
                headers: {
        ...
        function markAsRead(id) {
            fetch(`/recruiter/notifications/${id}/read`, {
                method: 'POST',
                headers: {
        ...
        function deleteNotification(id) {
            if (confirm('Êtes-vous sûr de vouloir supprimer cette notification ?')) {
                fetch(`/recruiter/notifications/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Accept': 'application/json',
        ...
        function markAllAsRead() {
            fetch('/recruiter/notifications/read-all', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Accept': 'application/json',
