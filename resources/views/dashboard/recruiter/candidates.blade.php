    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div class="bg-[#2b2b2b] border border-[#333333] rounded-2xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:transform hover:scale-[1.02]">
                ...
                <div class="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                    ...
                    @else
                        <div class="w-12 h-12 sm:w-16 sm:h-16 bg-[#00b6b4]/20 rounded-full flex items-center justify-center border-2 border-[#00b6b4] flex-shrink-0">
                            <span class="text-[#00b6b4] font-bold text-lg sm:text-xl">{{ $initials ?: 'U' }}</span>
                </div>
                    @endif
                <div class="flex-1 min-w-0">
                        {{-- Name --}}
                        <h3 class="text-lg sm:text-xl font-bold text-[#f5f5f5] mb-1 truncate">{{ is_string($candidate->name) ? $candidate->name : 'Candidat' }}</h3>
                        
                        {{-- Titre professionnel --}}
                        <p class="text-[#00b6b4] font-medium mb-2 text-sm sm:text-base truncate">{{ $profile->bio ? 'Développeur Frontend React' : 'Développeur Frontend' }}</p>
                        ...
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#9ca3af]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26 12,2"/></svg>
                                @endif
                            @endfor
                            <span class="text-xs sm:text-sm text-[#9ca3af]">({{ $rating }})</span>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3 sm:mb-4">
                    <p class="text-[#f5f5f5] text-xs sm:text-sm line-clamp-2">{{ is_string($profile->bio ?? null) ? $profile->bio : 'Développeur passionné avec 3 ans d\'expérience en React et TypeScript.' }}</p>
        </div>

                {{-- Profile Details with Icons --}}
                <div class="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
                    {{-- Location --}}
                <div class="flex items-center gap-2 text-xs sm:text-sm text-[#9ca3af]">
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#00b6b4] flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
                        <span class="truncate">{{ is_string($profile->city ?? null) ? $profile->city : 'Alger, Chéraga' }}</span>
                </div>

                    {{-- Experience --}}
                <div class="flex items-center gap-2 text-xs sm:text-sm text-[#9ca3af]">
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#00b6b4] flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="7" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path></svg>
                        <span class="truncate">{{ $profile->experience_years ?? '3 ans d\'expérience' }}</span>
                </div>

                    {{-- Education --}}
                <div class="flex items-center gap-2 text-xs sm:text-sm text-[#9ca3af]">
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#00b6b4] flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>
                        <span class="truncate">{{ $educationText ?: 'Master en Informatique' }}</span>
        </div>

                    {{-- Availability --}}
                <div class="flex items-center gap-2 text-xs sm:text-sm text-[#9ca3af]">
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-[#00b6b4] flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/></svg>
                        <span class="truncate">Disponible: {{ $profile->availability ?? 'Immédiate' }}</span>
            </div>
        </div>
            ...
            <div class="mb-3 sm:mb-4">
                ...
                <div class="flex flex-wrap gap-1.5 sm:gap-2">
                            ...
                            @endforeach
                            @if(count($skills) > 4)
                                <span class="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-[#333333] text-[#9ca3af] rounded-full text-xs font-medium">+{{ count($skills) - 4 }}</span>
                            @endif
                </div>
            </div>
                @endif
